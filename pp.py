    ...
    if sheet:
        try:
            if isinstance(data, list) and len(data) > 0:
                ...
                for row in data:
                    sheet.append_row(list(row.values()))  # Append each row
        except Exception as e:
...
def save_data(new_data):
    ...
    st.session_state.update_key = random.random()  # Force rerun
...
# Save employees to a local JSON file
...
def main():
    load_css("custom.css")  # Load custom CSS styles

    # Load employees and initialize session state
    ...
    with st.sidebar:
        selected = option_menu(
            ...
            options=["Home", "Orders", "Search", "Dashboard", "Settings"],
    ...
    # Home page for adding new orders
    if selected == "Home" and check_permission('Home'):
        ...
        region, kind = st.text_input("Enter the Region"), st.selectbox("Type of Product", ["smart watch", "airtag"])
    ...
    # Dashboard
    if selected == "Dashboard" and check_permission('Dashboard'):
        ...
        with col1:
            draw_circle(total_orders, "Total Orders", max_value=1500)
        with col2:
            draw_circle(pending_orders, "Pending Orders", max_value=150)

        col3, col4 = st.columns(2)
        with col3:
            draw_circle(completed_orders, "Completed Orders", max_value=150)
        with col4:
            draw_circle(delivered_orders, "Delivered Orders", max_value=1000)

        with st.expander("View Daily Revenue"):
...
# Functions to format orders for display
